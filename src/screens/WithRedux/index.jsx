...
import { getCountiesDetails } from '../../service/action';
...
const mapStateToProps = (state) => ({
	totalCountries: state.appReducer.totalCountries,
});

const WithoutRedux = ({ totalCountries }) => {

	const dispatch = useDispatch();
	const [state, updateState, onModalOpenClicked, closeModal, onDeleteItemClicked, onSearchChange] = countriesHook();

	const { filteredCountries, isModalOpen, searchKey, selectedViewValues } = state;

	React.useEffect(() => {
		if (totalCountries.length === 0) {
			dispatch(getCountiesDetails());
		} else {
			updateState({
				...state,
...
WithoutRedux.propTypes = {
	totalCountries: PropTypes.any.isRequired,
};

export default connect(mapStateToProps)(WithoutRedux);
